...
it('extracts augmented triliteral alif present verb roots without database', function (string $word, string $root): void {
    ...
    expect($analysis->root)->toBe($root)
        ...
        ->and($analysis->rootAnalysis->best->source)->toBe('rule:verb_triliteral_yfa3l')
        ->and($sources)->toContain('rule:verb_triliteral_yfa3l')
...
it('extracts augmented triliteral mufaala masdar roots without database', function (string $word, string $root): void {
    $analysis = ArabicAnalyzer::make()->analyze($word, StemmerModeEnum::Root);
